...
define inline function tracing-node?
  ...
  if (node.environment & node.environment.lambda)
    ...
    error("this node hasn't environment or lambda! %=", node)
  end;
end;
  ...
  if (unique-name)
    ...
    if (name-suffix)
      if (name-suffix == 0)
        ...
        as(<symbol>, concatenate(debug-string, "-", name-suffix.integer-to-string))
      end
    else
      *lambda-string-table*[debug-name] := pair(x, unique-name);
      as(<symbol>, concatenate(debug-string, "-", unique-name.size.integer-to-string))
    end
  else
